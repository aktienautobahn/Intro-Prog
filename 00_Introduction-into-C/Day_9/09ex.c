...
Um die Tests für dieses Blatt zu kompilieren und zu starten, führen Sie den folgenden Befehl aus:
 cc -std=c11 -g -Wall 09ex_test.c -o 09ex_test.o -lm && ./09ex_test.o

Um den Code mit `valgrind` auszuführen (unten erläutert), führen Sie den folgenden Befehl aus:
 cc -std=c11 -g -Wall 09ex_test.c -o 09ex_test.o -lm && valgrind ./09ex_test.o
...
uint32_t *return_pointer(uint32_t x) {
    // printf("pointer %p on the value %d", &x, x);
    // allocate memory , that is valid after function lifecycle
...
bool free_pancake(PileOfPancakes p) {
    ...
    // loop till the last layer and if existing free it
    while (p.further_layers != NULL) {
        // dereference pointer

        // iterate to the next layer
        PileOfPancakes *next_layer = p.further_layers;
        // assign the next layer to the current (next is now current)
        p = *next_layer;
        // free current layer
        free(next_layer);
...
// PileOfPancakes *first_layer = (PileOfPancakes *)malloc(sizeof(PileOfPancakes));
    ...
    for (int i = 1; i < n; i++) {
        ...
        // memory accocation failure handling
        if (new_layer == NULL) {
        ...
        *new_layer = p;
        // add new layer to the existing pile
        p.further_layers = new_layer;
        }
...
/*
Aufgabe 2c:
Rufen Sie `free` für alle Pointer auf, die Teil des gegebenen Pfannkuchens sind. Im Gegensatz zu Aufgabe 2b wird
der Pfannkuchen selbst nicht direkt übergegen, sondern lediglich ein Pointer auf einen Pfannkuchen.
Rufen Sie am Ende auch `free` für diesen Pointer auf.
Tipp: Das lässt sich mit sehr wenig neuem Code lösen, wenn man `free_pancake` schon implementiert hat und
wiederverwenden kann. Wir wollen nur noch einmal Unterschiede zwischen direkter Übergabe von Werten und von
...
uint16_t *create_dynamic_array(size_t x) {
    ...
    if (arr == NULL) {
    printf("Memory couln't be allocated. \n");
    exit(0);
    }
    else {
    for (int i = 0; i < x; i++) {
