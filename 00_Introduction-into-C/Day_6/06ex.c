...
*/
float normalize_value(int value, int max) {
    float result = (float)value / (max-1);
    return result;
}
...
Canvas swatch_red_green(Canvas c) {
    ...
    for (int x = 0 ; x < canvas_width(c); x++) {
        ...
        c = canvas_set_r(c, x, y, normalize_value(x, canvas_width(c)));
        c = canvas_set_g(c, x, y, normalize_value(y, canvas_height(c)));
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y) {
    for (int x = 0 ; x < canvas_width(c); x++) {
        for (int y = 0 ; y < canvas_height(c); y++) {
        c = canvas_set_channel(c, x, y, channel_x, normalize_value(x, canvas_width(c)));
        c = canvas_set_channel(c, x, y, channel_y, normalize_value(y, canvas_height(c)));
...
RGB rgb_to_gray(RGB color) {
    ...
    color.b = color.g = color.r = (color.r*0.2989 + color.g*0.587 + color.b*0.1140);
