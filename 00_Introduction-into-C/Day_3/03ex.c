...
int sum_of_doubly_divisibles(int n, int l, int m) {
    ...
    while (counter < n)
    {
        if (is_multiple_of(number, m) == 1 && is_multiple_of(number, l) == 1)  
        {
            summe += number;
            counter +=1;
        }
...
Canvas descending_diagonal(Canvas c) {
    int maxWidth = canvas_width(c) - 1;
    int maxHeight = canvas_height(c) - 1;
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height) {
    int maxWidth = canvas_width(c);
    int maxHeight = canvas_height(c);
    int max_x = x + width;
    int min_y = y - height;
    int x_counter = x;
    int y_start = y;

    for (x_counter; x_counter < max_x; x_counter++)
    {
        while (y > min_y)
        {
            if (x_counter < maxWidth && y < maxHeight && x_counter >= 0 && y >= 0) // überprüfung ob der Punk außerhalb von Canvas liegt.
            {
            c = canvas_set_black(c, x_counter, y);
            }
            y -= 1;
        }
        y = y_start;
...
Canvas draw_rectangle_via_corners(Canvas c, int x0, int y0, int x1, int y1) {
    ...
    int width = x1 - x0 + 1;
    int height = y0 - y1 + 1;
    // invoke function (don't have time to write a cleaner one)
