...
Canvas draw_odd_circle(Canvas c, int x, int y, int radius_from_middle) {
    ...
    // // printf("%d\n", y);
    // printf("%d\n", radius_from_middle);
    for (int x1 = 0; x1 < canvas_width(c); x1++) {   
        for (int y1 = 0; y1 < canvas_height(c); y1++) {   

        int dist = distance(x1, y1, x, y);
        // printf("%d", dist);
        if (radius_from_middle >= dist) {
            c = canvas_set_black(c, x1, y1);
        };
        }
        } 
...
int my_distance(int x0, int y0, int x1, int y1) {
    ...
    int distance = squareroot(a * a + b * b);
